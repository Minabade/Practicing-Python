class Product:
    ...
    def calculate_shipping_cost(self):
        shipping_cost: float = 10 * self.weight
        return f"shipping cost for {self.name}:${shipping_cost}"


class ElectronicProduct(Product):
    def __init__(self,name,price,stock,weight,warranty_years: int,brand: str):
        super().__init__(name,price,stock,weight)
        self.warranty_years  = warranty_years
        self.brand = brand
    
    def get_info(self):
        return f"Electronic Product: {self.name}: {self.brand}, Price: ${self.price}, Warranty: {self.warranty_years}years Stock: {self.stock}, Weight: {self.weight} kg"
...
class GrocreyProduct(Product):
    def __init__(self,name,price,stock,weight,expiration_date: str,is_perishable: bool):
        super().__init__(name,price,stock,weight)
...
Laptop = ElectronicProduct("Laptop",1500.00,10,2.5,3,"Dell")
Milk = GrocreyProduct("Milk",4.00,50,1.0,"2024-11-15","True")
